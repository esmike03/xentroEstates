...
<body>
    <div class="container mx-auto p-4">
        ...
        <form class=" text-sm" action="{{ route('admin.listings.update', $listing->id) }}" method="POST"
            ...
            <div class="grid grid-cols-2 gap-3">
                ...
                <div class="mb-4">
                    ...
                    <select name="category" id="category" class="w-full border rounded py-2 px-3">
                        ...
                        <option value="Land" {{ old('category', $listing->category) === 'Land' ? 'selected' : '' }}>
                            Land
                        </option>
                        <option value="Housing"
                            {{ old('category', $listing->category) === 'Housing' ? 'selected' : '' }}>
                            Housing</option>
                    </select>
                </div>


                <div class="mb-4 {{ old('category', $listing->category) === 'Housing' ? '' : 'hidden' }}"
                    id="housing-type-wrapper">
                    ...
                    <select name="housing_type" id="housing_type" class="w-full border rounded py-2 px-3">
                        ...
                        <option value="Office Property"
                            {{ old('housing_type', $listing->housing_type) === 'Office Property' ? 'selected' : '' }}>
                            Office
                            Property</option>
                        <option value="Residential"
                            {{ old('housing_type', $listing->housing_type) === 'Residential' ? 'selected' : '' }}>
                            Residential</option>
                        <option value="Other"
                            {{ old('housing_type', $listing->housing_type) === 'Other' ? 'selected' : '' }}>Other
                        </option>
                    </select>
                </div>

                <div class="mb-4 {{ old('housing_type', $listing->housing_type) === 'Other' ? '' : 'hidden' }}"
                    id="custom-housing-type-wrapper">
                    <label for="custom_housing_type" class="block text-gray-700 text-sm font-bold mb-2">Specify Housing
                        Type</label>
                    <input type="text" name="custom_housing_type" id="custom_housing_type"
                        class="w-full border rounded py-2 px-3"
                        value="{{ old('custom_housing_type', $listing->custom_housing_type) }}">
                </div>
                ...
                <div class="mb-4">
                    ...
                    <select name="type" id="type" class="w-full border rounded py-2 px-3">
                        <option value="sale" {{ old('type', $listing->type) == 'sale' ? 'selected' : '' }}>Sale
                        </option>
                        <option value="lease" {{ old('type', $listing->type) == 'lease' ? 'selected' : '' }}>Lease
                        </option>
                        <option value="rent" {{ old('type', $listing->type) == 'rent' ? 'selected' : '' }}>Rent
                        </option>
                    </select>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="bedrooms" id="bedrooms" class="w-full border rounded py-2 px-3"
                        value="{{ old('bedrooms', $listing->bedrooms) }}" min="0" step="1">
                </div>

                <div class="mb-4">
                    <label for="bathrooms" class="block text-gray-700 text-sm font-bold mb-2">Bathrooms</label>
                    <input type="number" name="bathrooms" id="bathrooms" class="w-full border rounded py-2 px-3"
                        value="{{ old('bathrooms', $listing->bathrooms) }}" min="0" step="1">
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="area" id="area" class="w-full border rounded py-2 px-3"
                        value="{{ old('area', $listing->area) }}" min="0">
                </div>

                <div class="mb-4">
                    <label for="status" class="block text-gray-700 text-sm font-bold mb-2">Status</label>
                    <select name="status" id="status" class="w-full border rounded py-2 px-3">
                        <option value="pending" {{ old('status', $listing->status) == 'pending' ? 'selected' : '' }}>
                            Pending</option>
                        <option value="active" {{ old('status', $listing->status) == 'active' ? 'selected' : '' }}>
                            Active
                        </option>
                        <option value="sold" {{ old('status', $listing->status) == 'sold' ? 'selected' : '' }}>Sold
                        </option>
                        <option value="leased" {{ old('status', $listing->status) == 'leased' ? 'selected' : '' }}>
                            Leased
                        </option>
                    </select>
                </div>
                ...
                <div class="mb-4">
                    ...
                    @if ($listing->image)
                        <img src="{{ asset('storage/' . $listing->image) }}" alt="Current Image" class="mt-2"
                ...
                document.addEventListener("DOMContentLoaded", function() {
                    ...
                    housingTypeSelect.addEventListener("change", function() {
                        if (this.value === "Other") {
                            ...
                            customHousingTypeWrapper.classList.add(
                                "hidden"); // Hide it if another option is selected
                            customHousingTypeInput.value = ""; // Reset value
                        }
                    });
                });
