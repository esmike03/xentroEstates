...
<body class="bg-gray-50 text-gray-900 font-[Ubuntu]" x-data="{ showModal: false }">
    ...
    @if ($subdivision)
        ...
        <div  x-init="$watch('showModal', value => { if (value) document.body.classList.add('overflow-hidden');
            ...
            <!-- Modal Overlay -->
            <div x-show="showModal" class="fixed inset-0 bg-black/60 bg-opacity-50 flex items-center justify-center z-50"
                    ...
                    <div id="blocks-container" class="mb-4">
                        @foreach ($subdivisions->blocks as $block)
                            <div class="bg-white rounded-lg px-6 mb-6">
                                ...
                                <p class="text-gray-700 mb-2"><strong>Block Area:</strong> {{ $block->block_area }} sq
                                    meters</p>
                                    ...
                                    @foreach ($block->houses as $house)
                                        <a
                                            ...
                                            <div
                                                ...
                                                <p class="text-sm"><strong>Lot Area:</strong> {{ $house->house_area }} m²</p>
                                                ...
                                                <p class="text-sm"><strong>Price per sqm:</strong>
                                                    @if ($catPrice)
                                                        <span class="text-gray-500">(₱{{ number_format($catPrice, 2) }}
                                                            per sqm)</span>
                                                    @else
                                                        Not available
                                                    @endif
